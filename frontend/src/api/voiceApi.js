...
export class VoiceSocket {
  ...
  startSession(opts = {}) {
    return this.send({
      type: WS_MESSAGE_TYPES.START_SESSION,
      ...opts,
    });
  }
  ...
  updateConfig(config) {
    return this.send({
      type: WS_MESSAGE_TYPES.UPDATE_CONFIG,
      config,
    });
  }
  ...
  sendUserSpeech(text, opts = {}) {
    return this.send({
      type: WS_MESSAGE_TYPES.USER_SPEECH,
      text,
      ...opts,
    });
  }
  ...
  sendInterruption(opts = {}) {
    return this.send({
      type: WS_MESSAGE_TYPES.INTERRUPTION,
      ...opts,
    });
  }

  // NEW: Server STT audio streaming
  startAudioStream(opts = {}) {
    return this.send({
      type: WS_MESSAGE_TYPES.AUDIO_STREAM_START,
      ...opts,
    });
  }
  ...
  stopAudioStream() {
    return this.send({
      type: WS_MESSAGE_TYPES.AUDIO_STREAM_END,
    });
  }
}